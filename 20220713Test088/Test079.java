...
○ 배열의 개요
   ...
   자바에서 제공하는 배열의 주의할 점
   1. 동질의 성격을 가져야 한다.
     - 변수를 각각 독립적으로 선언하는 건 메모리 공간에 불규칙적이게 할당된다.
	 - 하지만 배열은 한꺼번에, 인접해있는 형태로 한꺼번에 쓰게 된다.
	 - 자바는 그 공간을 쓸 수 있을지 없을지 확인한다음 쓰게 한다.
	 - 변수는 각각마다 주소를 가지고 있지만
	 - 배열은 처음 시작하는 방의 주소만 가지고 있다.

   2. 길이를 변화시킬 수 없다. 불변
     - 가변 배열 : 원래 4칸짜리 방인데 집을 다시 구하는 것이다.
	               메모리를 다시 잡고 안에 있던 것을 옮긴 뒤 주소를 바꾼다.
				   원래 있던 4칸짜리 방은 가비지 컬렉터가 수거해 간다.
