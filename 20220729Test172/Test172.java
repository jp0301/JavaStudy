...
○ 자바의 기본 입출력 개요
   ...
   - 자바에서의 모든 데이터의 입출력은 스트림(stream) 이라는 개념에 의해
	 ...
	 "흐르는 물"의 개념이며, 이는 연속된 일련의 데이터를 일커컫는다.

   - 데이터 입출 력 시 모든 데이터를 형태와 관계없이 일련의 흐름으로
...
※ 스트림(stream)의 특징
   ...
   - 스트림은 단방향이다.
     자바에서는 스트림은 읽기, 쓰기가 동시에 이루어지지 않는다.
...
public class Test172
{
	public static void main(String[] args) throws IOException
	{
		...
		while ((data = System.in.read()) != -1)
		{
			...
			//System.out.print(ch); // print는 문자기반스트림으로 내보낸다.
			//--==>>
			/*
			문자열 입력(종료:Ctrl+z)
			abcd
			...
			1234
			가나다라
			°¡³ª´?¶?                 // 바이트스트림으로 읽어들인 걸 문자기반으로 출력하니깐 문제가 발생
			^Z
			계속하려면 아무 키나 누르십시오 . . .
			*/
